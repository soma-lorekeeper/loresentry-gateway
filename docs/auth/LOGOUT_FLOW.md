...
Auth는 유효한 RT의 sid가 현재 세션과 같을 때만 삭제한다. 같은 세션에서 회전된 RT는
아직 만료 전이면 폐기할 수 있지만, 이전 로그인 세션의 RT로 새 세션을 지울 수 없다.
서명·클레임은 유효하지만 만료된 RT는 상태 변경 없이 완료한다. BFF의 쿠키 삭제 헤더만으로
서버 세션 삭제나 브라우저의 실제 쿠키 삭제를 보증하지 않는다.
