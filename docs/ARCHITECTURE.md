...
|---|---|---|
| 인증 | AT 검증, 인증 요청 연결, 브라우저 쿠키 처리 | 토큰 발급·RT 상태·Google 신원 검증은 Auth |
...
- `security`는 검증한 사용자 UUID를 웹 경계에 제공한다. 내부 서비스 호출이나 응답 조합을 수행하지 않는다.
...
현재 gateway에는 `web`, `client`, `config`와 서비스 연결 확인용 API가 있다.
실제 기능을 구현할 때 application과 security를 추가하고, 해당 기능부터 이 구조를 적용한다.
기존 연결 확인용 `UpstreamRelay`의 범용 Map 응답과 모든 연동 실패를 `502`로 묶는 처리는
완성된 기능 API의 응답 계약으로 재사용하지 않는다.
