...
class ContentApiRoutesTest {
    ...
    @Test void mapsProjectCreationToPublicDtoAndRelativeLocation() throws Exception {
        ...
        mvc.perform(post("/projects").requestAttr(com.loresentry.gateway.security.AccessTokenFilter.USER_ATTRIBUTE, USER)
    ...
    @Test void keepsSaveRevisionAndIdempotencyHeaders() throws Exception {
        ...
        mvc.perform(put("/files/"+ID+"/content").requestAttr(com.loresentry.gateway.security.AccessTokenFilter.USER_ATTRIBUTE, USER)
    ...
    @Test void rejectsAmbiguousHeadersBeforeCallingApplication() throws Exception {
        mvc.perform(put("/files/"+ID+"/content").requestAttr(com.loresentry.gateway.security.AccessTokenFilter.USER_ATTRIBUTE, USER).header("If-Match","1","2")
                .contentType(MediaType.APPLICATION_JSON).content("{}"))
            .andExpect(status().isBadRequest());
        verifyNoInteractions(service);
    }
    @Test void doesNotExposeArbitraryNamespacePathsOrMethods() throws Exception {
        mvc.perform(get("/projects/"+ID+"/internal").requestAttr(com.loresentry.gateway.security.AccessTokenFilter.USER_ATTRIBUTE, USER)).andExpect(status().isNotFound());
        mvc.perform(put("/projects/"+ID).requestAttr(com.loresentry.gateway.security.AccessTokenFilter.USER_ATTRIBUTE, USER).contentType(MediaType.APPLICATION_JSON).content("{}"))
            .andExpect(status().isMethodNotAllowed());
        verifyNoInteractions(service);
    }
    @Test void deleteHasNoResponseBodyAndUsesExplicitUser() throws Exception {
        mvc.perform(delete("/episodes/"+ID).requestAttr(com.loresentry.gateway.security.AccessTokenFilter.USER_ATTRIBUTE, USER)).andExpect(status().isNoContent()).andExpect(content().string(""));
    ...
    @Test void searchPassesDecodedQueryMeaningAndConditionalHeader() throws Exception {
        ...
        mvc.perform(get("/projects/"+ID+"/search").param("q","유리 & +?")
                .requestAttr(com.loresentry.gateway.security.AccessTokenFilter.USER_ATTRIBUTE, USER).header("If-None-Match","\"etag\""))
    ...
    void rejectsInvalidInputWithoutInternalCall(String body) throws Exception {
        mvc.perform(post("/projects").requestAttr(com.loresentry.gateway.security.AccessTokenFilter.USER_ATTRIBUTE, USER).contentType(MediaType.APPLICATION_JSON).content(body))
            .andExpect(status().isBadRequest()).andExpect(jsonPath("$.code").value("INVALID_REQUEST"))
            .andExpect(jsonPath("$.next_action").value("NONE"));
        verifyNoInteractions(service);
    }
    @Test void invalidPathPreservesNotFoundContract() throws Exception {
        mvc.perform(get("/files/not-a-uuid/content").requestAttr(com.loresentry.gateway.security.AccessTokenFilter.USER_ATTRIBUTE, USER))
    ...
    @Test void mapsSafeBusinessErrorAndPreservesConflictSnapshot() throws Exception {
        ...
        mvc.perform(put("/files/"+ID+"/content").requestAttr(com.loresentry.gateway.security.AccessTokenFilter.USER_ATTRIBUTE, USER).contentType(MediaType.APPLICATION_JSON).content("{}"))
