...
class OAuthControllerTest {
    ...
    TokenPair tokens(){return new TokenPair("access.payload.signature",NOW.plusSeconds(900),"refresh.payload.signature",NOW.plusSeconds(1209600));}
    ...
    @Test void callbackIgnoresHostReturnUrlAndProviderDescription() throws Exception {
        when(login.callback("request-id","secret-state","secret-code",null)).thenReturn(new LoginService.CallbackResult(tokens(),LoginService.Result.SUCCESS,true));
    ...
    void failureKeepsAuthenticationCookiesAndOnlyDeletesConsumedOAuth(String result,String consumed,String expected,int count) throws Exception {
        ...
        var response=mvc.perform(get("/auth/oauth/google/callback").cookie(new Cookie("ls_oauth","request-id"),new Cookie("ls_at","existing"),new Cookie("ls_rt","existing"))
    ...
    @Test void malformedSuccessPairDoesNotPartiallyReplaceAuthentication() throws Exception {
        when(login.callback(any(),any(),any(),any())).thenReturn(new LoginService.CallbackResult(
            new TokenPair("access.payload.signature",NOW.plusSeconds(100),"refresh.payload.signature",NOW),LoginService.Result.SUCCESS,true));
    ...
    @Test void productionResultIsFixedAndUnsafeAuthorizationUrlIsRejected() throws Exception {
        ...
        when(login.callback(any(),any(),any(),any())).thenReturn(new LoginService.CallbackResult(tokens(),LoginService.Result.SUCCESS,true));
        var response=mvc.perform(get("/auth/oauth/google/callback").param("state","state").param("code","code"))
